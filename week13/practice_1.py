...
for i in tqdm(range(window_size_half, height - window_size_half)): # target h
    for j in range(window_size_half, width - window_size_half): # target w
        ...
        compare_window = img_r[i - window_size_half:i + window_size_half + 1,
                         j - window_size_half:j + window_size_half + 1]
        min_cost = np.sum(np.abs(compare_window - target_window))
        # print(min_cost)
        horizontal_diff = 0
        for k in range(j, j+100):
            if k < width-window_size_half:
                # 비교대상(left)
                compare_window = img_r[i-window_size_half:i+window_size_half+1, k - window_size_half:k + window_size_half + 1]
                absolute_difference = np.sum(np.abs(compare_window - target_window))
...
disparity_arr = disparity_arr[window_size_half:height-window_size_half, window_size_half:width-window_size_half]
disparity_arr = (disparity_arr - np.min(disparity_arr)) / (np.max(disparity_arr) - np.min(disparity_arr))
print(disparity_arr)
cv2.imshow('Image', disparity_arr)
cv2.waitKey(0)
